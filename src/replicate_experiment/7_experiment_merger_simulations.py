...
def main(estimation_results_path, output_path, book_principal_components_path):

    # ------------------------------
    # Load data and model coefficients
    # ------------------------------

    input_path = get_file_path_from_config(path_type="EXPERIMENT_7", path="BOOK_INPUT_PATH")
    first_choice_data, _, _, book_titles = load_data_long(input_path, book_principal_components_path)
...
if __name__ == "__main__":
    date = "2025-09-15"
    estimation_results_dir = get_file_path_from_config(path_type="EXPERIMENT_7", path="ESTIMATION_RESULTS_DIR")
    estimation_results_path = os.path.join(estimation_results_dir, f"mixed_logit_results_{date}.xlsx")

    output_path = get_file_path_from_config(path_type="EXPERIMENT_7", path="OUTPUT_PATH")

    book_principal_components_path = get_file_path_from_config(path_type="EXPERIMENT_7", path="BOOK_PRINCIPAL_COMPONENTS_PATH")

    main(estimation_results_path=estimation_results_path, output_path=output_path, book_principal_components_path=book_principal_components_path)
