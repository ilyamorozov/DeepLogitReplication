...
data_dir = get_file_path_from_config(path_type="EXPERIMENT_9", path="DATA_DIR")
...
embeddings_path = get_file_path_from_config(path_type="EXPERIMENT_9", path="EMBEDDINGS_PATH")

# embedding model directories
...
all_pcs_explained_variance = {}

# Compute PCA for each model's embeddings and save the explained variance ratios
for model_name, embedding_matrix in embeddings.items():
    ...
    pca.fit_transform(embedding_matrix_scaled)

    # Store the principal component variance ratios
    all_pcs_explained_variance[model_name] = pca.explained_variance_ratio_


# Compute the cumulative summation of variance ratios across all 10 PCs 

explained_variation_cum = {}

for model_name, variance_ratios in all_pcs_explained_variance.items():
    if model_name == "vgg16_image":
        continue
    explained_variation_cum[model_name] = np.cumsum(variance_ratios)

_, ax = plt.subplots(figsize=(8, 6), nrows=2, ncols=2)
ax_dict = {
    "image": ax[0, 0],
    "title": ax[0, 1],
    "description": ax[1, 0],
...
#Getting the length of the first model's explained variance ratios list for setting axes later, setting x and y bounds
x_range = range(1, num_components+1)

#computing the maximum y value for y-axis limits
y_max = max(
    max(v)
    for k, v in explained_variation_cum.items()
    if k != "vgg16_image"
)

for model_name, variance_ratios in explained_variation_cum.items():
    if model_name == "vgg16_image":
        continue
    for key in ax_dict:
        if key in model_name:
            ax_dict[key].plot(x_range, variance_ratios, marker='o', lw=1, label=formatted_submodel_map[model_name], alpha=0.75, clip_on=False)
...
for key, axis in ax_dict.items():
    ...
    axis.set_xticklabels([f"PC{i}" for i in x_range], fontsize=8)
    axis.set_xlim(1,len(x_range))
    axis.set_ylim(0, y_max * 1.1)
    axis.legend(loc="best", fontsize="small")

plt.tight_layout()
plt.savefig(os.path.join(output_dir, "explained_variance_cumulative_by_type.png"))
plt.close()