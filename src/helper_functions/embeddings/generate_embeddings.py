...
def preprocess_texts(texts):
    ...
    """
    return [preprocess_text(text) for text in tqdm(texts, desc="Preprocessing texts")]
...
def embed_texts_use(texts, model_use, batch_size=64):
    ...
    """
    embeddings = []
    for i in tqdm(range(0, len(texts), batch_size), desc="Embedding texts with USE"):
...
def generate_image_embeddings(images, dir_path):
    ...
    for model_name, model in models.items():
        ...
        print(f"Feature extraction using {model_name} completed.")
...
def generate_text_embeddings(texts, dir_path):
    ...
    print(f"Found {len(review_keys)} to average embeddings from.")

    # Preprocess texts for each source
    texts_per_source = {}
    for source in text_sources:
        print(f"Processing text source: {source}")
    ...
    for model_name in ["COUNT", "TFIDF"]:
        print(f"Fitting {model_name} on combined corpus...")
        try:
    ...
    for source in text_sources:
        ...
        for model_name, model in embedding_models.items():
            print(f"Extracting features for source: {source} using model: {model_name}")
    ...
    for source, features_dict in features.items():
        for model_name, features in features_dict.items():
            output_file = os.path.join(dir_path, f"{source}_{model_name}_features.npy")
            np.save(output_file, features)

            output_file = os.path.join(dir_path, f"{source}_{model_name}_features.csv")
            features_df = pd.DataFrame(features)
